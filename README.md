...
[![CodeFactor](https://www.codefactor.io/repository/github/robfromboulder/viewmapper/badge)](https://www.codefactor.io/repository/github/robfromboulder/viewmapper)
...
```sql
create or replace view viewzoo.example.b as select * from viewzoo.example.a
create or replace view viewzoo.example.c as select * from viewzoo.example.b
create or replace view viewzoo.example.d as select * from viewzoo.example.b
```
...
> Please generate a mermaid diagram file for the viewzoo.example schema
```mermaid
graph TD
    node1["viewzoo.example.c"]
    node2["viewzoo.example.d"]
    node3["viewzoo.example.b"]
    node4["viewzoo.example.a"]
...
> Please generate a mermaid diagram file for the viewzoo.example schema, focusing on customer_360, using short view names for readability
```mermaid
graph TB
  ...
  "mcpServers": {
    "viewmapper-mcp-server": {
      ...
      "args": [
        "run", "-i", "--rm",
        "-e", "ANTHROPIC_API_KEY=sk-ant-...",
        "-e", "VIEWMAPPER_CONNECTION=jdbc:trino://host.docker.internal:8080?user=youruser",
        "robfromboulder/viewmapper-mcp-server:478b"
      ]
    }
  }
...
**When limiting discovery to a specific Trino catalog (for enterprise or regulated environments):**
...
> - For **multi-catalog** connections: use `jdbc:trino://hostname:port?user=username` and fully-qualified names in prompts
> - For **single-catalog** connections: use `jdbc:trino://hostname:port/catalog?user=username` and simple schema names in prompts
> - Use `host.docker.internal` instead of `localhost` for containerized MCP servers to connect to Trino running on a host port
